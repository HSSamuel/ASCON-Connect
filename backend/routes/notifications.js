...
// ==========================================
// 5. MANAGE NOTIFICATION (Soft Delete)
// ==========================================
// 1. GET NOTIFICATIONS
router.get("/", verify, async (req, res) => {
  try {
    const notifications = await Notification.find({
      $or: [
        { recipientId: req.user._id }, // Personal
        { isBroadcast: true },         // Broadcast
      ],
      deletedBy: { $ne: req.user._id }, // Exclude if user deleted it
    })
      .sort({ createdAt: -1 })
      .limit(50); // Limit to last 50 to keep UI fast

    // Add "isRead" flag dynamically for broadcasts
...
router.put("/read/:id", verify, async (req, res) => {
  try {
    ...
    if (notification.isBroadcast) {
      ...
      if (notification.recipientId.toString() !== req.user._id)
...
// 3. DELETE NOTIFICATION (Hard vs Soft Logic)
router.delete("/:id", verify, async (req, res) => {
  try {
    ...
    if (notification.isBroadcast) {
      // Broadcasts: Soft delete (hide for this user only)
      await Notification.findByIdAndUpdate(req.params.id, {
        $addToSet: { deletedBy: req.user._id },
      });
    } else {
      // Personal: Hard delete (Remove from DB entirely)
      if (notification.recipientId.toString() !== req.user._id)
        return res.status(403).json({ message: "Unauthorized" });
...
router.delete("/clear/all", verify, async (req, res) => {
  try {
    // A. Hard Delete Personal Notifications
    await Notification.deleteMany({ recipientId: req.user._id });

    // B. Soft Delete Broadcasts (Hide them)
